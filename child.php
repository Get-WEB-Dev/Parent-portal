...
if(isset($_POST["action"]))
{
	if($_POST["action"] == "fetch")
	{
		...
		if(isset($_POST["search"]["value"]))
		{
			...
			OR tbl_subject.subject_name LIKE "%'.$_POST["search"]["value"].'%" 
		...
		{
			$sub_array = array();
			$sub_array[] = '<img src="teacher_image/'.$row["teacher_image"].'" class="img-thumbnail" width="50" />';
	...
	if($_POST["action"] == 'Add' || $_POST["action"] == "Edit")
	{
		...
		if(empty($_POST["teacher_subject_id"]))
		{
			$error_teacher_grade_id = "subject is required";
			$error++;
		}
		else
		{
			$teacher_grade_id = $_POST["teacher_subject_id"];
		}
		...
		if($error > 0)
		{
			$output = array(
				...
				'error_teacher_subject_id'		=>	$error_teacher_subject_id,
		...
		else
		{
			if($_POST["action"] == 'Add')
			{
				...
				(teacher_name, teacher_address, teacher_emailid, teacher_password, teacher_qualification, teacher_doj, teacher_image, subject, teacher_grade_id) 
				SELECT * FROM (SELECT :teacher_name, :teacher_address, :teacher_emailid, :teacher_password, :teacher_qualification, :teacher_doj, :teacher_image, :teacher_subject_id, :teacher_grade_id) as temp 
				WHERE NOT EXISTS (
	...
	if($_POST["action"] == "edit_fetch")
	{
		...
		if($statement->execute())
		{
			...
			foreach($result as $row)
			{
				...
				$output["teacher_subject_id"] = $row["subject"];
